...
export function WholesaleTopbar({ onMenuClick, onCommandOpen, collapsed }: WholesaleTopbarProps): React.ReactElement {
  const pathname = usePathname();
  const router = useRouter();
  const crumbs = getWholesaleBreadcrumbs(pathname);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1 text-left text-xs">Search wholesale workspace…</span>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel>
              <div className="flex flex-col">
                <span className="text-sm font-medium text-foreground">Wholesale</span>
                <span className="text-xs font-normal text-muted-foreground">wholesaler@dropshop.nn</span>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => router.push("/wholesale/profile")}>
              Profile
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push("/wholesale/settings")}>
              Settings
            </DropdownMenuItem>
            <DropdownMenuSeparator />
