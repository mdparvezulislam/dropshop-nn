...
import { Monitor, Tablet, Smartphone, Sparkles, CheckCircle2, ShieldCheck, Tag, Star, Eye } from "lucide-react";
...
export function StudioLivePreview({ form, className = "" }: StudioLivePreviewProps) {
  ...
  return (
    <div className={`flex flex-col bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl ${className}`}>
      ...
      <div className="flex-1 bg-slate-950 p-4 overflow-auto flex justify-center items-start">
        <div
          className={`bg-white text-slate-900 transition-all duration-300 rounded-xl overflow-hidden shadow-2xl border border-slate-200 ${
            device === "desktop"
              ? "w-full max-w-4xl"
              : device === "tablet"
              ? "w-[640px]"
        ...
        >
          ...
          <div className="bg-slate-900 text-white px-4 py-2 text-xs flex justify-between items-center border-b border-slate-800">
            ...
            <span className="text-slate-400">bd.dropshop.com/products/{form.slug || "sample-slug"}</span>
          </div>
            ...
            <div className={`grid gap-6 ${device === "desktop" ? "grid-cols-2" : "grid-cols-1"}`}>
                    ...
                    {form.media.map((item, idx) => (
                      <div
                        ...
                        className={`w-14 h-14 rounded-lg overflow-hidden border-2 shrink-0 ${
                          item.isFeatured ? "border-amber-500" : "border-slate-200"
                        }`}
                      >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <h1 className="text-xl font-black text-slate-900 leading-snug mt-0.5">
                    {form.name || "Product Name Title"}
                  </h1>
                  <p className="text-xs text-slate-500 mt-1">SKU: {form.sku || "DS-PROD-0000"}</p>
                </div>
                ...
                {form.variants && form.variants.length > 0 && form.variants[0].sku && (
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-slate-700 block">Available Options:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {form.variants.map((v, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-1 bg-slate-100 border border-slate-300 rounded-md text-xs font-semibold text-slate-700"
                        >
                          {(v as any).name || v.sku}
                        </span>
                      ))}
            ...
            {form.specifications && form.specifications.length > 0 && (
              <div className="pt-4 border-t border-slate-200 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-3 rounded-lg border border-slate-200">
                  {form.specifications.map((spec: { key: string; value: string }, i: number) => (
