...
import { DollarSign, TrendingUp, Package, Building2, RefreshCw, ChevronRight } from "lucide-react";
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-xl bg-muted/40 p-4 border border-border">
              ...
              {costDiff !== 0 && (
                <span
                  className={cn(
                    "text-[11px] font-bold mt-0.5 flex items-center gap-0.5",
                    costDiff > 0 ? "text-destructive" : "text-success",
                  )}
                >
                  {costDiff > 0 ? "+" : ""}
                  {formatCentsToCurrency(costDiff, costData.currency)}
                </span>
              )}
            </div>
            <div className="rounded-xl bg-muted/40 p-4 border border-border">
              ...
              {landedDiff !== 0 && (
                ...
                >
                  {landedDiff > 0 ? "+" : ""}
                  {formatCentsToCurrency(landedDiff, costData.currency)}
                </span>
              )}
            </div>
            <div className="rounded-xl bg-muted/40 p-4 border border-border">
            ...
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-5">
                {expenseFields.map((f) => (
                  <div
                    key={f.label}
                    className="rounded-lg bg-muted/30 px-3 py-2 border border-border/60 text-center"
                  >
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pt-2 border-t border-border/60">
            <Button variant="outline" size="sm" onClick={loadCost} className="sm:self-start">
              <RefreshCw className="h-3.5 w-3.5 mr-1.5" /> Refresh
            </Button>
            <div className="flex flex-wrap gap-1.5">
              <Button variant="outline" size="sm">
                <Link
                  href={`/dashboard/costs?update=${productId}`}
              ...
              <Button variant="outline" size="sm">
                <Link
                  href={`/dashboard/costs?history=${productId}`}
                  className="flex items-center gap-1"
                >
              ...
              <Button variant="outline" size="sm">
                <Link
                  href={`/dashboard/costs?compare=${productId}`}
                  className="flex items-center gap-1"
                >
