...
import { ShoppingBag, Copy, Check, Code } from "lucide-react";
      ...
      <DialogContent className="max-w-2xl border border-border bg-card shadow-2xl rounded-2xl p-6">
        <DialogHeader className="flex flex-row items-center justify-between pb-2">
          <DialogTitle className="text-base font-bold flex items-center gap-2 text-foreground">
            <ShoppingBag className="h-4 w-4 text-primary" /> Google Merchant Center & JSON-LD Inspector
          </DialogTitle>
          <div className="flex items-center gap-1 rounded-lg border border-border bg-muted/40 p-1">
            <button
              ...
              className={`rounded-md px-2.5 py-1 text-xs font-bold transition-all ${
                tab === "jsonld" ? "bg-primary text-primary-foreground shadow-2xs" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              JSON-LD Schema
            </button>
            <button
              ...
              onClick={() => setTab("xml")}
              className={`rounded-md px-2.5 py-1 text-xs font-bold transition-all ${
                tab === "xml" ? "bg-primary text-primary-foreground shadow-2xs" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              XML Feed
            </button>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" className="gap-1.5 font-bold shadow-2xs" onClick={handleCopy}>
              {copied ? <Check className="h-3.5 w-3.5 text-success" /> : <Copy className="h-3.5 w-3.5" />} Copy Snippet
            </Button>
          </div>
        </div>
      </DialogContent>
