...
export class DepositService {
  ...
  async approveDeposit(depositId: string, approverId: string, notes?: string): Promise<Deposit> {
    return runInTransaction(async (session) => {
      ...
      const updated = await this.depositRepository.update(
        depositId,
        {
          status: "approved",
  ...
  async rejectDeposit(depositId: string, rejecterId: string, reason: string): Promise<Deposit> {
    return runInTransaction(async (session) => {
      const deposit = await this.depositRepository.findById(depositId, { session });
      if (!deposit) {
        throw new Error("Deposit request not found");
