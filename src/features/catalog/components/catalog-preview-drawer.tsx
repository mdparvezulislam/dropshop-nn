...
export function CatalogPreviewDrawer({ productId, onClose }: CatalogPreviewDrawerProps): React.ReactElement {
  ...
  const healthResult = useHealthScore({
    ...
    lowStockThreshold: "5",
    media: (product?.images ?? []).map((url: string, i: number) => ({ id: `m-${i}`, url, type: "image", isFeatured: i === 0 })),
    slug: product?.slug ?? "",
    ...
    inventorySku: "", inventoryBarcode: "", reservedStock: "0", incomingStock: "0", warehouseLocation: "", weight: "0.5",
    campaignPrice: "", metaKeywords: [], ogImage: "",
    bulletFeatures: [],
      ...
      <SheetContent className="w-full sm:max-w-md border-l border-border bg-card p-6 shadow-2xl overflow-y-auto ws-scroll">
        <SheetHeader className="pb-4 border-b border-border">
          <div className="flex items-center justify-between">
            <Badge variant={product?.status === "active" ? "success" : "warning"} size="xs" className="font-bold">
          ...
          <SheetTitle className="text-base font-extrabold text-foreground pt-2 line-clamp-2">
            {loading ? "Loading product..." : product?.title ?? product?.name ?? "Untitled Product"}
          </SheetTitle>
          <p className="text-xs font-mono text-muted-foreground">{product?.sku ?? "SKU-PROD"}</p>
        </SheetHeader>

        {loading ? (
          <div className="p-8 text-center text-xs text-muted-foreground animate-pulse">Loading preview…</div>
        ) : product ? (
          <div className="space-y-5 pt-4">
            ...
            <div className="aspect-[4/3] rounded-2xl border border-border bg-muted/30 overflow-hidden shadow-2xs">
              {product.images?.[0] ? (
                <img src={typeof product.images[0] === "string" ? product.images[0] : product.images[0].url} alt="Cover" className="h-full w-full object-cover" />
              ) : (
            ...
            <div className="p-3.5 rounded-xl border border-border bg-muted/20 space-y-2">
              <div className="flex items-center justify-between text-xs font-bold text-foreground">
                <span className="flex items-center gap-1.5"><Activity className="h-3.5 w-3.5 text-primary" /> Product Health Score</span>
                <span className="font-mono text-xs font-extrabold text-primary">{healthResult.score}/100</span>
              </div>
            ...
            <div className="space-y-1.5">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Multi-Tier Pricing Matrix</p>
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 rounded-xl border border-border bg-card">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase">Retail Customer</p>
                  <p className="text-sm font-extrabold font-mono text-foreground">৳{(product.retailPrice ?? product.price ?? 0).toLocaleString()}</p>
                </div>
                <div className="p-2.5 rounded-lg border border-border bg-card">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase">Cost Price</p>
                  <p className="text-sm font-extrabold font-mono text-muted-foreground">৳{(product.costPrice ?? 0).toLocaleString()}</p>
                </div>
              </div>
            </div>
            ...
            {/* Inventory Breakdown */}
            <div className="space-y-1.5">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Warehouse Allocation</p>
            ...
            <div className="pt-2 flex gap-2">
              <Button size="sm" variant="outline" className="flex-1 gap-1.5 font-bold text-xs" onClick={handleCopyUrl}>
