...
const WorkflowDefinitionSchema = new Schema(
  {
    ...
    category: {
      ...
      enum: ["notification", "logistics", "finance", "inventory", "order", "cms", "analytics", "communication", "webhook", "system"],
    },
    status: {
      type: String, required: true,
    ...
    trigger: {
      type: { type: String, required: true, enum: ["event", "schedule", "manual", "webhook", "api"] },
      eventType: String,
      cron: String,
      webhookUrl: String,
      apiEndpoint: String,
    ...
    rules: [{
      ...
      conditions: [{
        field: String,
        operator: { type: String, enum: ["eq", "neq", "gt", "gte", "lt", "lte", "in", "nin", "contains", "startsWith", "endsWith", "exists", "regex"] },
      ...
      actions: [{
        id: String,
        type: { type: String, enum: ["send_notification", "send_email", "send_sms", "create_shipment", "update_order", "update_inventory", "create_wallet_transaction", "generate_invoice", "generate_report", "trigger_analytics_event", "execute_webhook", "delay", "wait", "branch", "stop_workflow"] },
    ...
    lastRunAt: Date,
    lastRunStatus: { type: String, enum: ["completed", "failed", "running", "pending", "cancelled", "retrying", "paused", "timeout"] },
...
export const WorkflowDefinitionModel = mongoose.models.WorkflowDefinition
