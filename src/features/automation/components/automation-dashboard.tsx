    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <AutomationMetricCard
          ...
          value={data.scheduledJobs}
          icon={<Clock className="h-3.5 w-3.5" />}
          index={1}
        />
        <AutomationMetricCard
          label="Today's Executions"
          value={data.todayExecutions}
        ...
        <AutomationMetricCard
          ...
          value={data.deadLetterQueue}
          icon={<AlertCircle className="h-3.5 w-3.5" />}
          index={6}
        />
        <AutomationMetricCard
          label="Avg Execution Time"
          value={data.avgExecutionTime}
          format="duration"
          icon={<Clock className="h-3.5 w-3.5" />}
          index={7}
        />
                ...
                {Object.entries(data.workflowStatusCounts).map(([status, count]) => (
                  ...
                  >
                    <span className="flex items-center gap-2 text-sm capitalize">
                      <span
                        className={`h-2 w-2 rounded-full ${status === "active" ? "bg-emerald-500" : status === "paused" ? "bg-amber-500" : status === "draft" ? "bg-gray-500" : "bg-rose-500"}`}
                      />
                      {status}
                    </span>
              ...
              <ul className="space-y-1">
                {data.recentExecutions.slice(0, 8).map((exec) => (
                  ...
                  >
                    ...
                    <Badge
                      variant="outline"
                      className={`text-[10px] ${statusColors[exec.status] ?? ""}`}
                    >
                      {exec.status}
                    </Badge>
                  </li>
