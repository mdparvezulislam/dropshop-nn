...
export function ResellerTopbar({ onMenuClick, onCommandOpen, collapsed }: ResellerTopbarProps): React.ReactElement {
  const pathname = usePathname();
  const router = useRouter();
  const crumbs = getResellerBreadcrumbs(pathname);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1 text-left text-xs">Search reseller workspace…</span>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel>
              <div className="flex flex-col">
                <span className="text-sm font-medium text-foreground">My Shop</span>
                <span className="text-xs font-normal text-muted-foreground">reseller@dropshop.nn</span>
              </div>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => router.push("/reseller/settings")}>
              Settings
            </DropdownMenuItem>
            <DropdownMenuSeparator />
