...
import {
  DollarSign, TrendingUp, Percent, Shield, Lock, Unlock,
...
export default function ProductPricingPanel({ product, onRecalculate, onOverrideToggle, className }: Props): React.ReactElement {
  const [recalculating, setRecalculating] = React.useState(false);
  const [locked, setLocked] = React.useState(product.isLocked);

  const handleRecalculate = async () => {
    if (locked) { toast.error("Price is locked. Unlock first."); return; }
  ...
  const stockLabel = product.availableStock > 10 ? "in_stock" : product.availableStock > 0 ? "low_stock" : "out_of_stock";
  const marginTone = product.profitMargin > 20 ? "success" : product.profitMargin > 5 ? "warning" : "danger";
        ...
        <div className="flex-1 p-4 sm:p-5 space-y-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex gap-1 shrink-0">
              ...
              <button onClick={() => { navigator.clipboard.writeText(product.productId); toast.success("ID copied"); }}
                className="h-8 w-8 flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted" title="Copy Product ID">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <PriceCell label="খরচ (Cost)" value={product.baseCostPrice} currency={product.currency} />
            <PriceCell label="Retail" value={product.sellingPrice} currency={product.currency} bold />
            <PriceCell label="Wholesale" value={product.wholesalePrice} currency={product.currency} />
          ...
          {product.promotionalPrice && product.promotionalPrice > 0 && (
            <div className="bg-warning/10 border border-warning/20 rounded-lg px-3 py-2 flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-warning">{formatCentsToCurrency(product.promotionalPrice, product.currency)}</span>
              {product.comparePrice && product.comparePrice > product.sellingPrice && (
                <span className="text-[11px] text-muted-foreground line-through">{formatCentsToCurrency(product.comparePrice, product.currency)}</span>
              )}
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span className={cn("font-semibold", marginTone === "success" ? "text-success" : marginTone === "warning" ? "text-warning" : "text-destructive")}>
            ...
            <StatusChip label={stockLabel.replace("_", " ")} tone={stockLabel === "in_stock" ? "success" : stockLabel === "low_stock" ? "warning" : "danger"} />
            {product.isOverridden && <Badge variant="warning" size="xs">Manual Override</Badge>}
            {product.pricingProfile && <Badge variant="soft" size="xs">{product.pricingProfile}</Badge>}
          ...
          <div className="flex flex-wrap gap-2 pt-1 border-t border-border">
            <Button variant="outline" size="sm" onClick={handleRecalculate} loading={recalculating} disabled={locked}>
            ...
            <Link href={`/dashboard/pricing/${product.id}`}>
              <Button variant="outline" size="sm"><DollarSign className="h-3.5 w-3.5" /> Edit Pricing</Button>
            </Link>
            <button onClick={() => { setLocked(!locked); onOverrideToggle?.(product.productId, !locked); }}
              className={cn("inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs font-semibold border transition-colors",
                locked ? "border-warning/30 bg-warning/10 text-warning" : "border-border hover:bg-muted"
              )}>
...
function PriceCell({ label, value, currency, bold }: { label: string; value: number; currency: string; bold?: boolean }): React.ReactElement {
  if (!value) return <div />;
  return (
    <div>
