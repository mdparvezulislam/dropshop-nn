...
export async function checkPriceProtectionAction(
  productId: string,
  ...
  return {
    success: true,
    data: await engine.checkPriceProtection(
      productId,
      proposedPrice,
      role,
      quantity ?? 1,
...
export async function simulatePricingAction(
  input: Parameters<PricingEngineService["simulatePrice"]>[0],
) {
  const session = await auth();
  checkPermission(session, "Pricing.View");
...
export async function resolveMoqPriceAction(
  productId: string,
  quantity: number,
  variantSku?: string,
) {
...
export async function recalculatePricesAction(
  productId: string,
  variantSku?: string,
): Promise<{
  success: boolean;
  data?: { sellingPrice: number; wholesalePrice: number; resellerPrice: number };
  error?: string;
}> {
  const session = await auth();
  checkPermission(session, "Pricing.Update");
