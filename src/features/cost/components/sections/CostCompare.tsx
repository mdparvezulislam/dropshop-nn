      ...
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          ...
          <Badge variant="soft" size="xs">
            v{versionA.versionNumber} vs v{versionB.versionNumber}
          </Badge>
        </CardTitle>
      </CardHeader>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left py-2 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
                Field
              </th>
              <th className="text-right py-2 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
          ...
          <tbody className="divide-y divide-border">
            <CompareRow
              label="Cost Price"
              a={versionA.costPrice}
              b={versionB.costPrice}
              currency={currency}
            />
            <CompareRow
              label="Landed Cost"
              a={versionA.landedCost}
              b={versionB.landedCost}
              currency={currency}
            />
            <CompareRow
              label="Profit"
              a={versionA.profitAmount ?? 0}
              b={versionB.profitAmount ?? 0}
              currency={currency}
            />
            <tr>
              <td className="py-2 font-medium">Profit Margin</td>
              <td className={cn("py-2 text-right font-mono", marginColor(versionA.profitMargin))}>
                {versionA.profitMargin?.toFixed(1)}%
              </td>
              <td className={cn("py-2 text-right font-mono", marginColor(versionB.profitMargin))}>
                {versionB.profitMargin?.toFixed(1)}%
              </td>
            ...
            <tr>
              ...
              <td className="py-2 text-right text-xs text-muted-foreground">
                {versionA.supplier?.supplierName ?? "—"}
              </td>
              <td className="py-2 text-right text-xs text-muted-foreground">
                {versionB.supplier?.supplierName ?? "—"}
              </td>
  ...
  return (
    <tr>
      ...
      <td className="py-2 text-right font-mono font-semibold">
        {formatCentsToCurrency(b, currency)}
      </td>
      <td
        className={cn(
          "py-2 text-right font-mono text-xs",
          isPos ? "text-rose-500" : diff < 0 ? "text-emerald-500" : "",
        )}
      >
        {diff !== 0
          ? `${isPos ? "+" : ""}${formatCentsToCurrency(Math.abs(diff), currency)} (${diffPct > 0 ? "+" : ""}${diffPct}%)`
          : "—"}
      </td>
    </tr>
  );
