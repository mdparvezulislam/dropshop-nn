...
import { Building2, CheckCircle2, ArrowRight, Shield, Calculator, FileText, Users } from "lucide-react";
...
export function WholesaleSection(): React.ReactElement {
  return (
    <section className="py-16 lg:py-24 bg-white border-b border-border/50" aria-label="Wholesale Solutions">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            <p className="text-base text-muted-foreground leading-relaxed">
              Designed specifically for Bangladeshi shop owners, electronics retailers, and regional distributors. Access wholesale tier pricing, flexible Minimum Order Quantities (MOQ), and nationwide delivery.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              {WHOLESALE_BENEFITS.map((item) => {
                ...
                return (
                  <div key={item.title} className="p-4 rounded-xl bg-muted/40 border border-border/60 space-y-1.5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Link href="/become-wholesale-partner">
                <Button size="lg" className="h-12 px-6 text-sm font-bold bg-amber-600 hover:bg-amber-700 text-white shadow-md">
                  Apply as Wholesale Buyer
                  <ArrowRight className="h-4 w-4 ml-2" />
                </Button>
              </Link>
              <Link href="/contact">
                <Button size="lg" variant="outline" className="h-12 px-6 text-sm font-bold border-amber-600/30 text-amber-700 hover:bg-amber-50">
          ...
          >
            <div className="rounded-3xl bg-gradient-to-br from-amber-500/10 via-amber-500/5 to-transparent p-6 sm:p-8 border border-amber-500/20 shadow-xl space-y-6">
              <div className="flex items-center justify-between border-b border-amber-500/20 pb-4">
                <div>
                  <h3 className="text-lg font-extrabold text-foreground">Live Wholesale Pricing Matrix</h3>
                  <p className="text-xs text-muted-foreground">Sample tier discount table for verified buyers</p>
                </div>
              ...
              <div className="p-4 rounded-xl bg-card border border-border/60 space-y-2 text-xs">
                ...
                <p className="text-muted-foreground leading-relaxed">
                  Wholesale accounts gain access to bulk checkout pipelines, automated PO management, and fast courier dispatch across all 64 districts.
                </p>
              </div>
            </div>
          </motion.div>
