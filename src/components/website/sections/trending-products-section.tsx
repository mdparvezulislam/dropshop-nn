...
export function TrendingProductsSection({
  ...
  description = "এই মুহূর্তে সবচেয়ে জনপ্রিয় প্রোডাক্টসমূহ",
}: TrendingProductsSectionProps): React.ReactElement | null {
  if (products.length === 0) return null;

  return (
    <section
      className="py-6 sm:py-10 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100"
        ...
        <div className="flex items-end justify-between mb-5 sm:mb-6">
          <div>
            <div className="flex items-center gap-2">
              <Flame className="h-5 w-5 text-amber-500 fill-amber-500" aria-hidden />
              <h2
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-bold mt-0.5">{description}</p>
          </div>
