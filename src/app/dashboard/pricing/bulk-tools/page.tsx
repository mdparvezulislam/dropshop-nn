...
import {
  bulkPricingOperationAction,
  listAllPricingForSearchAction,
...
const operations = [
  {
    type: "increase_percent" as const,
    label: "Increase by %",
    icon: ArrowUp,
    color: "text-success",
  },
  {
    type: "decrease_percent" as const,
    label: "Decrease by %",
    icon: ArrowDown,
...
export default function BulkToolsPage(): React.ReactElement {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      for (const [k, v] of Object.entries(filter)) {
        if (v.trim()) bf[k] = v.trim();
      }
      const res = await bulkPricingOperationAction(bf, {
        type: opType as any,
        value: opValue,
        field: field as any,
      });
      if (res.success && res.data) {
        ...
        toast.success(`Updated ${(res.data as any).updated} products`);
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title="Bulk Pricing Tools"
        description="বাল্ক মূল্য আপডেট — ক্যাটাগরি, ব্র্যান্ড, সাপ্লায়ার অনুযায়ী"
        icon={Layers}
      />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">
              Filter Products{" "}
              <span className="text-xs font-normal text-muted-foreground">ফিল্টার</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {filterOptions.map((opt) => (
                <FormField key={opt.key} label={`${opt.label} ID`}>
                  <Input
                    value={filter[opt.key] ?? ""}
                    onChange={(e) => setFilter({ ...filter, [opt.key]: e.target.value })}
                    placeholder={`Filter by ${opt.label.toLowerCase()}`}
                  />
                </FormField>
              ))}
            </div>
            ...
            {result && (
              <Card
                className={result.errors.length > 0 ? "border-warning/30" : "border-success/30"}
              >
        ...
        <div className="space-y-3">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">MOQ Pricing Tiers</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {[
                ...
                <button
                  key={i}
                  className="w-full text-left text-xs py-2 px-3 rounded-lg hover:bg-muted transition-colors flex items-center gap-2"
                >
