...
type Profile = {
  ...
  slug: string;
  description?: string;
  markupRules: Array<{ channel: string; markupType: string; markupValue: number }>;
...
export default function PricingProfilesPage(): React.ReactElement {
  ...
  const [form, setForm] = React.useState<{
    ...
    roundPriceTo: 1,
    isDefault: false,
    markupRules: [{ channel: "retail", markupType: "percentage", markupValue: 40 }],
  });
  ...
  const openEdit = (p: Profile) => {
    ...
    setForm({
      ...
      roundPriceTo: p.roundPriceTo ?? 1,
      isDefault: p.isDefault,
      markupRules: p.markupRules,
    });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await updatePricingProfileAction(editing.id, form);
        toast.success("Profile updated");
      } else {
        await createPricingProfileAction({ ...form, discountRules: [] });
  ...
  const handleDelete = async (id: string) => {
    try {
      await deletePricingProfileAction(id);
  ...
  const addRule = () =>
    setForm({
      ...
      markupRules: [
        ...form.markupRules,
        { channel: "retail", markupType: "percentage", markupValue: 30 },
      ],
    });
  ...
  const removeRule = (i: number) =>
    setForm({ ...form, markupRules: form.markupRules.filter((_, idx) => idx !== i) });
    ...
    },
    {
      ...
      cell: (r) => (
        <div className="flex gap-1 flex-wrap">
          {r.markupRules.map((mr) => (
            <Badge key={mr.channel} variant="soft" size="xs">
              {mr.channel}: {mr.markupValue}%
            </Badge>
          ))}
        </div>
      ),
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        description="পুনর্ব্যবহারযোগ্য প্রোফাইল — Standard, Premium, Budget, Wholesale, Reseller সহ"
        icon={Layers}
        action={
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4 mb-4">
        {profiles.slice(0, 8).map((p) => (
          <Card
            key={p.id}
            className={`hover:border-primary/40 ${p.isDefault ? "border-primary/50 ring-1 ring-primary/20" : ""}`}
          >
            <CardContent className="p-3">
              ...
              <div className="mt-1 text-[10px] text-muted-foreground">
                {p.markupRules.map((r) => `${r.channel}: ${r.markupValue}%`).join(", ")}
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <DataTable
        ...
        emptyDescription="Create reusable pricing profiles to apply consistent markup rules across products."
      />

      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Profile" : "Create Pricing Profile"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              {form.markupRules.map((rule, i) => (
                <div key={i} className="grid grid-cols-4 gap-2 items-center">
                  ...
                  >
                    {["retail", "wholesale", "reseller", "distributor"].map((c) => (
