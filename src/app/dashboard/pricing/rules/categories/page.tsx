...
import {
  listCategoryOverridesAction,
  createCategoryOverrideAction,
  updateCategoryOverrideAction,
  deleteCategoryOverrideAction,
...
export default function CategoryPricingPage(): React.ReactElement {
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await updateCategoryOverrideAction(editing.id, form);
        toast.success("Category pricing updated");
      } else {
        await createCategoryOverrideAction(form);
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteCategoryOverrideAction(id);
  ...
  const columns: DataTableColumn<Override>[] = [
    {
      id: "name",
      header: "Category",
      cell: (r) => <div className="font-medium">{r.categoryName}</div>,
    },
    {
      id: "markup",
      header: "মার্কআপ",
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        description="ক্যাটাগরি অনুযায়ী মূল্য নির্ধারণ — Power Bank, Speaker, Earbuds সহ সব ক্যাটাগরি"
        icon={Tags}
        action={
          <Button size="sm" onClick={openCreate}>
            <Plus className="h-3.5 w-3.5" /> Add Category Rule
          </Button>
        }
      />
      <DataTable
        ...
        data={overrides}
        loading={loading}
        emptyTitle="No category overrides"
        emptyDescription="Set different pricing rules for different product categories."
      />

      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Category Pricing" : "Add Category Pricing"}</DialogTitle>
          </DialogHeader>
