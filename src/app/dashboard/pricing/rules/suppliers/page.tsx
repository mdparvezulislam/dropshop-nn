...
import {
  listSupplierPricingRulesAction,
  ...
  const columns: DataTableColumn<Rule>[] = [
    {
      id: "name",
      header: "Supplier",
      cell: (r) => <div className="font-medium">{r.supplierName}</div>,
    },
    {
      id: "markup",
      header: "মার্কআপ",
    ...
    },
    {
      ...
      header: "Lead Cost",
      hideOnMobile: true,
      cell: (r) => <span className="tabular-nums">{r.leadCost ? `৳${r.leadCost}` : "-"}</span>,
    },
    {
      id: "handling",
      header: "Handling",
      hideOnMobile: true,
      cell: (r) => (
        <span className="tabular-nums">{r.handlingFee ? `৳${r.handlingFee}` : "-"}</span>
      ),
    },
    {
      id: "status",
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        description="সাপ্লায়ার অনুযায়ী মূল্য নির্ধারণ — মার্কআপ ও খরচ কনফিগার করুন"
        icon={Building2}
      />
      <DataTable
        columns={columns}
        data={rules}
        loading={loading}
        emptyTitle="No supplier pricing rules"
        emptyDescription="Supplier-specific pricing will appear here once configured."
      />
    </div>
  );
}
