...
export default function GlobalRulesPage(): React.ReactElement {
  ...
  const [form, setForm] = React.useState({ name: "", channel: "retail", markupType: "percentage", markupValue: 40, minMarginPercent: 5, maxDiscount: 70, roundPriceTo: 1, priority: 100, isActive: true });
  ...
  const openCreate = () => { setEditing(null); setForm({ name: "", channel: "retail", markupType: "percentage", markupValue: 40, minMarginPercent: 5, maxDiscount: 70, roundPriceTo: 1, priority: 100, isActive: true }); setDialogOpen(true); };
  const openEdit = (r: Rule) => { setEditing(r); setForm({ name: r.name, channel: r.channel, markupType: r.markupType, markupValue: r.markupValue, minMarginPercent: r.minMarginPercent ?? 5, maxDiscount: r.maxDiscount ?? 70, roundPriceTo: r.roundPriceTo ?? 1, priority: r.priority, isActive: r.isActive }); setDialogOpen(true); };
  ...
  const handleDelete = async (id: string) => {
    try { await deleteGlobalRuleAction(id); toast.success("Rule deleted"); load(); }
    catch (err: any) { toast.error(err.message); }
  };

  const columns: DataTableColumn<Rule>[] = [
    { id: "name", header: "Rule Name", cell: (r) => (
      <div><div className="font-medium">{r.name}</div>
      <Badge variant={r.isActive ? "success" : "muted"} size="xs">{r.isActive ? "Active" : "Draft"}</Badge></div>
    )},
    { id: "channel", header: "Channel", cell: (r) => <Badge variant="soft" size="sm">{r.channel.replace(/_/g, " ")}</Badge> },
    { id: "markup", header: "মার্কআপ", cell: (r) => (
      <span className="font-semibold tabular-nums">{r.markupType === "percentage" ? `${r.markupValue}%` : `${r.markupValue}`}</span>
    )},
    { id: "minMargin", header: "Min মার্জিন", hideOnMobile: true, cell: (r) => <span>{r.minMarginPercent ?? "-"}%</span> },
    { id: "maxDiscount", header: "Max Discount", hideOnMobile: true, cell: (r) => <span>{r.maxDiscount ?? 100}%</span> },
    { id: "priority", header: "Priority", hideOnMobile: true, cell: (r) => <span className="font-mono text-xs">{r.priority}</span> },
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        icon={Shield}
        action={<Button size="sm" onClick={openCreate}><Plus className="h-3.5 w-3.5" /> Create Rule</Button>}
      />
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4 mb-4">
        {CHANNELS.map((ch) => (
          <div key={ch} className="rounded-xl border border-border/80 bg-card p-3">
            <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{ch.replace(/_/g, " ")}</div>
      ...
      <DataTable columns={columns} data={rules} loading={loading} emptyTitle="No global rules" emptyDescription="Create your first global pricing rule to set channel-level markups." />
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="grid gap-4 py-4">
            <FormField label="Rule Name" required>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Standard Retail Markup" />
            </FormField>
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField label="Markup Type">
                <select value={form.markupType} onChange={(e) => setForm({ ...form, markupType: e.target.value })}
                  className="h-9.5 w-full rounded-lg border border-input bg-card px-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Markup Value" required>
                <Input type="number" value={form.markupValue} onChange={(e) => setForm({ ...form, markupValue: Number(e.target.value) })}
              ...
              <FormField label="Round To">
                <select value={form.roundPriceTo} onChange={(e) => setForm({ ...form, roundPriceTo: Number(e.target.value) })}
                  className="h-9.5 w-full rounded-lg border border-input bg-card px-3 text-sm">
                  {[1, 5, 10, 50, 100, 500, 1000].map((v) => <option key={v} value={v}>Nearest {v}</option>)}
                </select>
              </FormField>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Min Margin %">
                <Input type="number" value={form.minMarginPercent} onChange={(e) => setForm({ ...form, minMarginPercent: Number(e.target.value) })} />
              </FormField>
