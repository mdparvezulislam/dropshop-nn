...
import {
  listBrandOverridesAction,
  createBrandOverrideAction,
  updateBrandOverrideAction,
  deleteBrandOverrideAction,
...
export default function BrandPricingPage(): React.ReactElement {
  ...
  const openEdit = (r: Override) => {
    ...
    setForm({
      ...
      channel: r.channel,
      markupType: r.markupType as any,
      markupValue: r.markupValue,
      minProfitPercent: r.minProfitPercent ?? 5,
      maxDiscountPercent: r.maxDiscountPercent ?? 60,
    });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await updateBrandOverrideAction(editing.id, form);
        toast.success("Brand pricing updated");
      } else {
        await createBrandOverrideAction(form);
        toast.success("Brand pricing created");
      }
      setDialogOpen(false);
      load();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteBrandOverrideAction(id);
  ...
  const columns: DataTableColumn<Override>[] = [
    ...
    },
    {
      ...
      header: "Min লাভ",
      hideOnMobile: true,
      cell: (r) => <span>{r.minProfitPercent ?? "-"}%</span>,
    },
    {
      id: "maxDisc",
      header: "Max Discount",
      hideOnMobile: true,
      cell: (r) => <span>{r.maxDiscountPercent ?? 100}%</span>,
    },
    {
      id: "actions",
      header: "",
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title="Brand Pricing"
        description="ব্র্যান্ড অনুযায়ী মূল্য নির্ধারণ — Baseus, Xiaomi, Joyroom, Hoco সহ সব ব্র্যান্ড"
        icon={ShoppingBag}
        action={
          <Button size="sm" onClick={openCreate}>
            <Plus className="h-3.5 w-3.5" /> Add Brand Rule
          </Button>
        }
      />
      <DataTable
        ...
        data={overrides}
        loading={loading}
        emptyTitle="No brand overrides"
        emptyDescription="Configure per-brand pricing rules for your catalog."
      />
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Brand Pricing" : "Add Brand Pricing"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField label="Markup Type">
                <select
                  ...
                  onChange={(e) => setForm({ ...form, markupType: e.target.value as any })}
                  className="h-9.5 w-full rounded-lg border border-input bg-card px-3 text-sm"
                >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField label="Min Profit %">
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, minProfitPercent: Number(e.target.value) })}
                />
              </FormField>
            </div>
