  ...
  const columns: DataTableColumn<Approval>[] = [
    {
      ...
      cell: (r) => (
        <div>
          <div className="font-medium capitalize">{r.entityType.replace(/_/g, " ")}</div>
    ...
    },
    {
      id: "requested",
      header: "Requested By",
      cell: (r) => <span>{r.requestedByName ?? "System"}</span>,
    },
    {
      id: "reason",
      header: "Reason",
    ...
    },
    {
      ...
      header: "Date",
      hideOnMobile: true,
      cell: (r) => <span className="text-xs">{new Date(r.createdAt).toLocaleDateString()}</span>,
    },
    {
      id: "status",
      header: "স্ট্যাটাস",
      cell: (r) => (
        <Badge
          variant={
            r.status === "pending" ? "warning" : r.status === "approved" ? "success" : "destructive"
          }
          size="sm"
        >
        ...
        r.status === "pending" ? (
          <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="h-8 w-8 flex items-center justify-center rounded-md text-success hover:bg-success/10"
            >
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        description="মূল্য অনুমোদন ওয়ার্কফ্লো — পেন্ডিং অনুমোদন পর্যালোচনা করুন"
        icon={CheckCircle}
        action={
      ...
      <Dialog open={!!reviewDialog} onOpenChange={(o) => !o && setReviewDialog(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {reviewDialog?.action === "approve" ? "Approve" : "Reject"} Price Change
            </DialogTitle>
          </DialogHeader>
          <div className="py-4">
