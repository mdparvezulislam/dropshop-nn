...
import { ArrowLeft, RefreshCw, Clock, CheckCircle, SkipForward, AlertTriangle, Plus } from "lucide-react";

const PRIORITY_LABELS: Record<string, string> = { low: "Low", normal: "Normal", high: "High", urgent: "Urgent" };
const TYPE_LABELS: Record<string, string> = {
  call: "Call", message: "Message", delivery_reminder: "Delivery Reminder",
  payment_reminder: "Payment Reminder", custom: "Custom",
};
...
export default function FollowUpsPage(): React.ReactElement {
  ...
  const [form, setForm] = React.useState({ orderId: "", title: "", dueDate: "", priority: "normal", type: "call", customerName: "", customerPhone: "" });
  ...
  const handleCreate = async () => {
    ...
    if (res.success) { toast.success("ফলো-আপ তৈরি হয়েছে"); setShowForm(false); setForm({ orderId: "", title: "", dueDate: "", priority: "normal", type: "call", customerName: "", customerPhone: "" }); load(); }
    else toast.error(res.error || "ব্যর্থ");
  };
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowForm(!showForm)}><Plus className="h-4 w-4 mr-1" /> New</Button>
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        ...
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-emerald-400" /><span className="text-xs text-muted-foreground">Completed</span></div>
            <p className="text-xl font-bold text-emerald-400">{stats.completed ?? 0}</p>
          </CardContent>
        </Card>
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-rose-400" /><span className="text-xs text-muted-foreground">Overdue</span></div>
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}
                className="h-9 rounded-lg border border-border bg-card px-3 text-xs text-muted-foreground">
                <option value="call">Call</option><option value="message">Message</option><option value="delivery_reminder">Delivery Reminder</option>
                <option value="payment_reminder">Payment Reminder</option><option value="custom">Custom</option>
              </select>
            </div>
      ...
      <div className="flex gap-3">
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
          ...
          <option value="pending">Pending</option><option value="completed">Completed</option><option value="skipped">Skipped</option>
        </select>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/50 bg-muted/30">
                <th className="p-3 text-left text-xs font-semibold text-muted-foreground">শিরোনাম</th>
                ...
                <th className="p-3 text-left text-xs font-semibold text-muted-foreground">শেষ তারিখ</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={item.id} className={`border-b border-border/30 hover:bg-muted/20 ${isOverdue(item) ? "bg-rose-500/5" : ""}`}>
                  ...
                  <td className="p-3"><Badge variant={item.priority === "urgent" ? "destructive" : item.priority === "high" ? "warning" : "default"}>{PRIORITY_LABELS[item.priority]}</Badge></td>
                  <td className={`p-3 text-sm ${isOverdue(item) ? "text-rose-400 font-semibold" : "text-muted-foreground"}`}>
                    {new Date(item.dueDate).toLocaleDateString()}
                  </td>
                  <td className="p-3"><Badge variant={getVariant(item.status)}>{item.status}</Badge></td>
                  <td className="p-3 text-sm text-muted-foreground">{item.assignedToName ?? "-"}</td>
                  <td className="p-3 text-right">
                    {item.status === "pending" && (
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="outline" onClick={() => handleStatus(item.id, "completed")}>
                          <CheckCircle className="h-3.5 w-3.5 mr-1" /> Complete
                        </Button>
                        <Button size="sm" variant="secondary" onClick={() => handleStatus(item.id, "skipped")}>
