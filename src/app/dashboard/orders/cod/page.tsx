...
import {
  ...
  settleCodAction,
  getCodStatsAction,
} from "@/features/order/actions/cod-actions";
...
export default function CodCenterPage(): React.ReactElement {
  ...
  const load = async () => {
    setLoading(true);
    const [listRes, statsRes] = await Promise.all([listCodAction(1, 50), getCodStatsAction()]);
  ...
  const filtered = items.filter(
    (i) =>
      !search ||
      i.orderNumber?.includes(search) ||
      i.trackingNumber?.includes(search) ||
      i.courierName?.includes(search),
  );
      ...
      <Card className="border-border/50 bg-card/50">
        <CardContent className="p-3">
          <div className="relative">
            ...
            <Input
              placeholder="Search order, tracking, courier..."
              value={search}
              ...
              {loading ? (
                ...
                filtered.map((item: any) => (
                  <tr key={item.id} className="border-b border-border/30 hover:bg-muted/20">
                    ...
                    <td
                      className={`p-3 text-right text-sm font-medium ${item.difference !== 0 ? "text-rose-400" : "text-emerald-400"}`}
                    >
                      ...
                      {item.settlementStatus === "pending" && (
                        <div className="flex justify-end gap-1">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleReconcile(item.id, item.expectedAmount)}
                          >
