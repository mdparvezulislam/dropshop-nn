...
} from "@/features/order/actions/warranty-actions";
import {
  getWarrantyHumanLabel,
  type WarrantyStatus,
} from "@/features/order/domain/warranty-entity";
              ...
              {loading ? (
                ...
                items.map((w: any) => (
                  <tr key={w.id} className="border-b border-border/30 hover:bg-muted/20">
                    ...
                    <td className="p-3">
                      <Badge variant={getVariant(w.status)}>
                        {getWarrantyHumanLabel(w.status)}
                      </Badge>
                    </td>
                    <td className="p-3 text-right">
                      {w.status === "requested" && (
                        <>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleTransition(w.id, "approved")}
                          >
                          ...
                          <Button
                            size="sm"
                            variant="destructive"
                            onClick={() => handleTransition(w.id, "rejected")}
                            className="ml-1"
                          >
                      ...
                      {w.status === "approved" && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleTransition(w.id, "in_repair")}
                        >
                      ...
                      {w.status === "in_repair" && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleTransition(w.id, "repaired")}
                        >
