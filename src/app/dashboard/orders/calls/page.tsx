...
import { ArrowLeft, RefreshCw, Phone, CalendarDays, CheckCircle, XCircle, Plus } from "lucide-react";

const OUTCOME_LABELS: Record<string, string> = {
  reached: "Reached", not_reached: "Not Reached", busy: "Busy",
  switched_off: "Switched Off", wrong_number: "Wrong Number",
  call_back_later: "Call Back Later", completed: "Completed",
};
...
export default function CallsPage(): React.ReactElement {
  ...
  const [form, setForm] = React.useState({ orderId: "", customerName: "", customerPhone: "", staffName: "", duration: 0, outcome: "reached", notes: "" });
  ...
  const load = async () => {
    ...
    if (statsRes.success && statsRes.data) {
      setStats({
        ...
        not_reached: (statsRes.data.byOutcome.not_reached ?? 0) + (statsRes.data.byOutcome.busy ?? 0) + (statsRes.data.byOutcome.switched_off ?? 0) + (statsRes.data.byOutcome.wrong_number ?? 0),
      });
    }
    setLoading(false);
  };

  React.useEffect(() => { load(); }, [outcomeFilter]);

  const handleCreate = async () => {
    const res = await createCallLogAction({ ...form, staffId: "system", duration: Number(form.duration) });
    if (res.success) { toast.success("কল লগ সংরক্ষিত"); setShowForm(false); setForm({ orderId: "", customerName: "", customerPhone: "", staffName: "", duration: 0, outcome: "reached", notes: "" }); load(); }
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowForm(!showForm)}><Plus className="h-4 w-4 mr-1" /> New Call</Button>
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-blue-400" /><span className="text-xs text-muted-foreground">Total</span></div>
            <p className="text-xl font-bold">{stats.total ?? 0}</p>
          </CardContent>
        </Card>
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-amber-400" /><span className="text-xs text-muted-foreground">Today</span></div>
            <p className="text-xl font-bold text-amber-400">{stats.today ?? 0}</p>
          </CardContent>
        </Card>
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-emerald-400" /><span className="text-xs text-muted-foreground">Reached</span></div>
            <p className="text-xl font-bold text-emerald-400">{stats.reached ?? 0}</p>
          </CardContent>
        </Card>
        <Card className="border-border/50 bg-card/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2"><XCircle className="h-4 w-4 text-rose-400" /><span className="text-xs text-muted-foreground">Not Reached</span></div>
        ...
        <Card className="border-border/50 bg-card">
          <CardContent className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Input placeholder="Order ID" value={form.orderId} onChange={(e) => setForm({ ...form, orderId: e.target.value })} />
              ...
              <Input placeholder="Staff Name" value={form.staffName} onChange={(e) => setForm({ ...form, staffName: e.target.value })} />
              <Input type="number" placeholder="Duration (sec)" value={form.duration || ""} onChange={(e) => setForm({ ...form, duration: Number(e.target.value) })} />
            ...
            <Button onClick={handleCreate} className="mt-2">Save</Button>
          </CardContent>
        </Card>
      )}
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={item.id} className="border-b border-border/30 hover:bg-muted/20">
                  ...
                  <td className="p-3 text-sm">{item.customerName}<br /><span className="text-xs text-muted-foreground">{item.customerPhone}</span></td>
                  <td className="p-3 text-sm">{item.staffName}</td>
                  <td className="p-3 text-right text-sm">{item.duration}s</td>
                  <td className="p-3"><Badge variant={item.outcome === "reached" || item.outcome === "completed" ? "success" : "secondary"}>{OUTCOME_LABELS[item.outcome] ?? item.outcome}</Badge></td>
                  <td className="p-3 text-sm text-muted-foreground max-w-[200px] truncate">{item.notes ?? "-"}</td>
                  <td className="p-3 text-sm text-muted-foreground">{new Date(item.callTime ?? item.createdAt).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
