...
export default function CreateOrderPage(): React.ReactElement {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!customer.name.trim() || !customer.phone.trim()) {
      toast.error("গ্রাহকের নাম এবং ফোন প্রয়োজন");
      return;
    }
    if (items.some((i) => !i.productName.trim())) {
      toast.error("সব পণ্যের নাম দিন");
      return;
    }
      ...
      const res = await createManualOrderAction({
        ...
        items: items
          ...
          .map((i) => ({
            ...
            currency: "BDT",
            pricingSource:
              source === "reseller" ? "reseller" : source === "wholesale" ? "wholesale" : "retail",
          })),
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6">
      <div className="max-w-4xl mx-auto space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Card className="border-border/50 bg-card">
            <CardHeader>
              <CardTitle className="text-sm font-semibold">গ্রাহকের তথ্য *</CardTitle>
            </CardHeader>
          ...
          <Card className="border-border/50 bg-card">
            <CardHeader>
              <CardTitle className="text-sm font-semibold">ডেলিভারি ঠিকানা *</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-4 sm:grid-cols-2">
              <Input
          ...
          <Card className="border-border/50 bg-card">
            ...
            <CardContent className="space-y-3">
              {items.map((item, idx) => (
                <div
                  key={item.key}
                  className="flex flex-wrap items-end gap-2 p-3 rounded-lg border border-border/50 bg-muted/10"
                >
                  ...
                  <div className="w-20">
                    ...
                    <Input
                      ...
                      value={item.quantity}
                      onChange={(e) =>
                        updateItem(item.key, "quantity", parseInt(e.target.value) || 1)
                      }
                      className="h-8 text-xs"
                    />
                  </div>
                  <div className="w-24">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateItem(item.key, "unitSellingPrice", parseFloat(e.target.value) || 0)
                      }
                      className="h-8 text-xs"
                    />
                  </div>
                  <div className="w-24">
                    ...
                    <Input
                      ...
                      min={0}
                      step={0.01}
                      value={item.unitCostBasis}
                      onChange={(e) =>
                        updateItem(item.key, "unitCostBasis", parseFloat(e.target.value) || 0)
                      }
                      className="h-8 text-xs"
                    />
                  </div>
          ...
          <Card className="border-border/50 bg-card">
            <CardHeader>
              <CardTitle className="text-sm font-semibold">প্রাইসিং সারাংশ</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <Card className="border-border/50 bg-card">
            <CardHeader>
              <CardTitle className="text-sm font-semibold">নোট (ঐচ্ছিক)</CardTitle>
            </CardHeader>
            <CardContent>
              <textarea
                rows={2}
