...
import { listExchangesAction, updateExchangeStatusAction } from "@/features/order/actions/exchange-actions";
import { getExchangeHumanLabel, type ExchangeStatus } from "@/features/order/domain/exchange-entity";
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border/50 bg-muted/30">
                <th className="p-3 text-left text-xs font-semibold text-muted-foreground">এক্সচেঞ্জ #</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-12 text-center text-muted-foreground">কোনো এক্সচেঞ্জ নেই</td></tr>
              ) : items.map((e: any) => (
                <tr key={e.id} className="border-b border-border/30 hover:bg-muted/20">
                  ...
                  <td className="p-3"><Badge variant={getVariant(e.status)}>{getExchangeHumanLabel(e.status)}</Badge></td>
                  <td className="p-3 text-right">
                    {e.status === "requested" && (
                      <>
                        <Button size="sm" variant="outline" onClick={() => handleTransition(e.id, "approved")}>
                          Approve
                        </Button>
                        <Button size="sm" variant="destructive" onClick={() => handleTransition(e.id, "rejected")} className="ml-1">
                          <XCircle className="h-3.5 w-3.5 mr-1" /> Reject
                        </Button>
                      </>
                    )}
                    {e.status === "approved" && (
                      <Button size="sm" variant="outline" onClick={() => handleTransition(e.id, "pickup_scheduled")}>
                        <Truck className="h-3.5 w-3.5 mr-1" /> Schedule Pickup
                      </Button>
                    )}
                    {e.status === "picked_up" && (
                      <Button size="sm" variant="outline" onClick={() => handleTransition(e.id, "replacement_shipped")}>
